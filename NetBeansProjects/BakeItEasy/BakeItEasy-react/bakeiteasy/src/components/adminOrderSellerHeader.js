...
export function AdminOrderSellerHeaderNonMemo({ oId }) {
  ...
  const routeChangeToSellerProfile = (sId) => {
    if (sId) {
      let path = "/adminViewSellerProfile?id=" + sId;
      navigate(path);
    }
  };
...
export const AdminOrderSellerHeader = memo(AdminOrderSellerHeaderNonMemo);
